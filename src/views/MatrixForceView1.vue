<template>
  ...
  <svg style="width: 1600; height: 600" ref="matrixView">
    ...
    <g class="gContentMatrixView1">
      <g>
        <line v-for="item in link" :x1="item.x1" :y1="item.y1" :x2="item.x2" :y2="item.y2" stroke="#ddd"
          stroke-width="0.3"></line>
      </g>
      <rect v-for="item in rectList" :x="item.x" :y="item.y" :width="item.width" :height="item.height"
        fill="transparent" stroke="black" stroke-width="1" />
      <circle v-for="item in start_data" :r="4" :cx="item.x" :cy="item.y" :fill="`url(#halfMV_${item.id})`"
        @click="handleClick(item, 0)" :stroke="getStrokeColor(item.id)" stroke-width="1"></circle>
      <circle v-for="item in matrix_1_0_data" :r="item.r || 4" :cx="item.x" :cy="item.y"
        :fill="`url(#halfMV_${item.id})`" @click="handleClick(item, 1)" :stroke="getStrokeColor(item.id)"
        stroke-width="0.5"></circle>
      <circle v-for="item in matrix_2_1_data" :r="item.r || 4" :cx="item.x" :cy="item.y"
      ...
      handler(newVal, oldVal) {
        ...
        let newResult = result
          .map((item) => {
            ...
            let targetNode = find(
              [...this.start_data, ...this.matrix_1_0_data, ...this.matrix_2_1_data],
              { id: target }
            )
            let sourceNode = find(
              [...this.start_data, ...this.matrix_1_0_data, ...this.matrix_2_1_data],
    ...
    initHierarchy() {
      ...
      this.svg.call(
        d3
          ...
          .on('zoom', (event) => {
            let { transform } = event
            d3.select('.gContentMatrixView1').transition().duration(10).attr('transform', transform)
          })
      )
    },
