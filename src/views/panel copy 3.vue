      ...
      <div style="display: flex; width: 270px;">
        ...
        <div style="margin-right: 5px">
          <v-responsive class="mx-auto" width="70" >
            <v-text-field
                v-model="this.epoch1"
                hide-details="auto" label="epoch1"
            ></v-text-field>
          </v-responsive>
        </div>
        ...
        <div style="margin-right: 10px">
          <v-responsive class="mx-auto" width="60">
            <v-text-field
                v-model="this.layer1"
                hide-details="auto" label="layer1"
            ></v-text-field>
          </v-responsive>
        </div>
        ...
        <div style="margin-right: 5px">
          <v-responsive class="mx-auto" width="70" >
            <v-text-field
                v-model="this.epoch2"
                hide-details="auto" label="epoch2"
            ></v-text-field>
          </v-responsive>
        </div>
        ...
        <div>
          <v-responsive class="mx-auto" width="60">
            <v-text-field
                v-model="this.layer2"
                hide-details="auto" label="layer2"
            ></v-text-field>
          </v-responsive>
        </div>
      </div>
    ...
    <div class="container" style="width: 1100px; height: 450px">
      <div style="margin-bottom: 0">
        <v-col class="py-2" cols="12">
          ...
          <v-btn-toggle
              v-model="text" color="deep-purple-accent-3" rounded="0" group>
            <v-btn @click="this.defaultDraw">position</v-btn>
            <v-btn @click="this.trueDraw">true label</v-btn>
      ...
      <svg ref="scatterPlot0" class="scatterPlot0"></svg>
...
export default {
  ...
  methods: {
    ...
    brushDraw() {
      ...
      // if(this.brush1 != "") {
      //   console.log("ss")
      //   this.brush1.on('start', null)
      //   this.brush1.on('end', null)
...
console.log(scatter1)
      ...
      // 创建SVG元素
      const svg0Width = 600
      const svg0Height = svg0Width
      const svg0 = d3.select(this.$refs.scatterPlot0)
          .attr("width", svg0Width)
          .attr("height", svg0Height)
          .attr("viewBox", [-svg0Width, -svg0Height, svg0Width*4, svg0Height*4])
          .attr("style", "max-width: 100%; height: auto; height: intrinsic;");


      // 创建散点图0的分组
      const scatter0Group = svg0.append("g")
          .attr("transform", `translate(${margin.left}, ${margin.top})`);

      const colors0 = d3.scaleOrdinal(d3.schemeAccent);

      // 绘制散点图1
      const graphWidth = svg0Width;
      const graphHeight = svg0Height;
      const renderNodes = []
      let edges = sf.allEdges
      if (sf.graph0Initialized === false) {
        for (let i = 0; i < sf.allNodes.length; i++) {
          const n = {
        ...
        console.log(renderNodes)
          let sim = d3.forceSimulation(renderNodes)
            //连接力，力的大小类似弹簧
            .force('link', d3.forceLink(edges).id((d) => d.id).distance(30).iterations(1))
            //电荷力，排斥或吸引
            .force('charge', d3.forceManyBody().strength(-20))
            //碰撞力，防止节点重合
            .force(
                'collide',
                d3.forceCollide().radius((d) => d.r).iterations(1).strength(5)
            ) 
            .force("x", d3.forceX())
            .force("y", d3.forceY());
            // .force('x', d3.forceX().x(graphWidth / 2).strength(0.3))
            // .force('y', d3.forceY().y(graphHeight / 2).strength(0.3))
          for (
            let i = 0,
                n =
                    0 ||
        ...
        let lines = svg0
            ...
            .enter()
            .append("line")
            .attr("x1", (d) => renderNodeMap[d.source.id].x)
            .attr("x2", (d) => renderNodeMap[d.target.id].x)
            .attr("y1", (d) => renderNodeMap[d.source.id].y)
            .attr("y2", (d) => renderNodeMap[d.target.id].y)
            .attr("stroke", "#999")
            .attr("stroke-width", 1)
            .attr('stroke-opacity', 0.3)
            .attr('stroke-linecap', 'round')
            .attr('data-index', (_, i) => i)
       const node = svg0.selectAll("circle")
            .data(renderNodes)
            .enter()
            .append("circle")
            .attr("cx", (d) => d.x)
            ...
            .attr('data-index', (_, i) => i)
            .on('click', handleSimGraphMouseClick)
        sf.graph0Initialized = true
      }
      ...
      function handleSimGraphMouseClick(event, d) {
        const nodeId = d.id
        sf.click_id = nodeId  // 记录点击的节点
        console.log('handleSimGraphMouseClick: ', nodeId)

        const relatedLinks = edges.filter(link => link.source.id === nodeId || link.target.id === nodeId)
        console.log(nodeId, relatedLinks)
        ...
        // 高亮与该节点相关的边
        svg0.selectAll('line').filter(function (d) {
      ...
      function getPointColor1(d) {
        const a = labScale(d.x);
        const b = labScale(d.y);
        const l = 60;
      ...
      if (sf.brush === true) {
        ...
        function brushed1(event) {
          ...
          // const actualX1 = x1
          // const actualY1 = y1
          //console.log(actualX0, actualY0, actualX1, actualY1);
  ...
  mounted() {
    // watch(() => this.nodes, () => {
    //   this.draw(); // 监视nodes数组的变化，并在内容发生变化时调用draw函数
    // });
