        ...
        <div class="container" style="width: 0; flex: 1">
            <div style="margin-bottom: 0">
                ...
                <div style="display: flex">
                    <v-btn @click="this.clickDraw"
                        style="margin-left: 10px; margin-right: 20px; margin-top: 10px">reset</v-btn>
                    <v-switch label="brush" v-model="brush" style="margin: 0; padding: 0" @click="brushInit"></v-switch>
                </div>
            </div>

            <div>
                <div style="width: 100%">
                    <chart-plot ref="childComponent1" :brushList="brushList" :dataList="dataList" :width="scatterPlotW"
                        :height="0.5 * scatterPlotW" @click-node="(data) => clickNode(data)"></chart-plot>
                </div>
                <div style="width: 50%" v-if="false">
                    <chart-plot ref="childComponent2" v-if="false" :selectActNode="selectActNode2" :dataList="dataList"
                        @click-node="(data) => clickNode(data)"></chart-plot>
                </div>
            </div>
            <force-guidance :result_edge="forceData.result_edge" :result_nodes="forceData.result_nodes"
                :brushNode="brushNode"></force-guidance>
...
export default {
    ...
    methods: {
        ...
        showMatrixChart() {
            ...
            let nodeArrObj = [scatterSvg.selectAll('.scatter1.highlight'), scatterSvg.selectAll('.scatter2.highlight')].map((nodes) => {
                let nodeObj = {}
